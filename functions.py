...
# Patch DepthwiseConv2D deserialization to ignore 'groups' if present
...
def patched_from_config(cls, config):
    config.pop('groups', None)  # Remove unsupported key
...
def img_predict(path, crop):
    ...
    data = data * 1.0 / 255
    print(os.path.join(BASE_DIR, 'models', 'DL_models', f'{crop}_model.h5'))
...
def get_crop_recommendation(item):
    ...
    crops = {'Apple, Kidney Beans, Peas ': 1, 'Banana, Green Gram, Groundnut': 2, 'Blackgram, Jowar, Maize': 3, 'Chickpea, Wheat, Barley ': 4, 'Coconut, Ginger, Turmeric': 5, 'Coffee, Cardamom': 6, 'Cotton, Soybean Turmeric': 7, 'Grapes, Onion, Garlic': 8, 'Jute, Mustard, Groundnut': 9, 'Kidneybeans, Bajra, Wheat ': 10, 'Lentil, Oats, Barley': 11, 'Maize, Black Gram, Green Gram': 12, 'Mango, Tomato, Chilli': 13, 'Mothbeans,  Jowar, Bajra': 14, 'Mungbean, Rice, Wheat': 15, 'Muskmelon, Onion, Garlic': 16, 'Orange, Coriander, Spinach': 17, 'Papaya, Chilli, Tomato': 18, 'Pigeonpeas, Ragi, Bajra': 19, 'Pomegranate, Jowar, Wheat': 20, 'Rice, Tomato, Garlic': 21, 'Watermelon, Spinach Chilli': 22}
    crops = list(crops.keys())
